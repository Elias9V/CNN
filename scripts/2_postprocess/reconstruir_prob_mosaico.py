...
como NumPy (float32 0-1) suavizado.
...
import cv2  # ← necesario para aplicar suavizado
...
def main():
    ...
    for idx, fname in enumerate(files):
        ...
        canvas[r * PATCH_SIZE:(r + 1) * PATCH_SIZE,
               c * PATCH_SIZE:(c + 1) * PATCH_SIZE] = prob

    # 🔧 Suavizar para eliminar bordes entre parches
    canvas = cv2.GaussianBlur(canvas, ksize=(5, 5), sigmaX=1)

    os.makedirs(os.path.dirname(OUT_NPY), exist_ok=True)
    np.save(OUT_NPY, canvas)
    print("✅ Mosaico de probabilidades suavizado guardado →", OUT_NPY)
